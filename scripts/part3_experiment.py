...
def run_experiment(config: Part3Config, *, save_single_comparison: bool | None = None) -> Path:
	...
	sample_time_seconds = perf_counter() - sample_start
	generated = generated.detach().cpu().numpy()
	total_time_seconds = train_time_seconds + sample_time_seconds
	...
	metadata = {
		...
		"final_train_loss": f"{result.losses[-1]:.6f}" if result.losses else "nan",
	}
	(run_dir / "metadata.txt").write_text(
		"\n".join(f"{k}: {v}" for k, v in metadata.items()), encoding="utf-8"
	)
	np.save(run_dir / "loss_history.npy", np.asarray(result.losses, dtype=np.float32))
